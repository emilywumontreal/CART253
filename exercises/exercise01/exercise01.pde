...
*/
// Set CIRCLE_SPEED value into 7. This value is unchangable while the coding is running.
...
/* setup function 
   inicialize variables and paint the window, only run once while the code runs. 
*/
void setup() {
  // initialize a window which size is 640 pixels in width,460 pixels in height.
  size(640, 480);
  // set values to variable circleX, circleY in the middle of window. The value of circleVX and circleVY is 7.
  circleX = width/2;
  circleY = height/2;
...
/* draw function
   drawing eclipses boncing inside the window  
